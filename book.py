...
def create_book():
    ...
    last_book = book_collection.find_one(sort=[("book_id", -1)])  # หา book_id ที่มากที่สุด
    new_book_id = (last_book["book_id"] + 1) if last_book else 1  # เพิ่มค่าถัดไป
 
    new_book = {
        "book_id": new_book_id,  # กำหนดค่า book_id
        "title": data["title"],
        "author": data["author"],
